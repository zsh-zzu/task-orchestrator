...
> 示例：针对"迁移学习是否适用于当前小样本数据集"，同时让 Agent A 提出支持论据、Agent B 从统计角度提出质疑，最后由 Orchestrator 裁决。

### 辩论共识 / Debate & Consensus

多 Agent 就某一议题进行结构化辩论，每轮提出论点与反驳，最终收敛到共识方案。

> 示例：在系统架构选型上，Agent A 倾向微服务而 Agent B 倾向单体方案，经过两轮辩论后达成折中的模块化单体方案。

### 分层编排 / Hierarchical Orchestration

将大任务分解为多个层级，每个层级由不同的 Lead Agent 负责编排其下属子任务，形成树状指挥结构。

> 示例：Orchestrator 统筹一个论文项目，将"实验"子层交给 Agent A 编排（包含数据准备、模型训练、结果分析），将"图表"子层交给 Agent B 编排。
...
任务计划中的 `agent` 字段需与 OpenClaw 中已注册的 Agent ID 对应。以下为示例映射，请根据实际配置调整：

| agent 值 | 角色示例 | 职责 |
|-----------|---------|------|
| `orchestrator` | 总管 Agent | 统筹协调、结果汇总 |
| `research-agent` | 研究类 Agent | 文献调研、方案设计 |
| `dev-agent` | 开发类 Agent | 代码开发、脚本工具 |
| `analyst-agent` | 分析类 Agent | 数据分析、技术面 |
| `content-agent` | 内容类 Agent | 文案撰写、内容运营 |
| `support-agent` | 辅助类 Agent | 日常记录、情绪支持 |
| `specialist-agent` | 专业类 Agent | 特定领域深度分析 |
